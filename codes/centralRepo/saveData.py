...
def parseDeapFile(dataPath, chans = list(range(32))):
    '''
    Parse the deap data file and return an epoched data. 
    ...
    labelPath : str
        path to the labels mat file.
    epochWindow : list, optional
        time segment to extract in seconds. The default is [0,2].
    chans  : list : channels to select from the data. 
    ...
    data : an EEG structure with following fields:
        ...
        c: list of channels - can be list of ints. 
    '''
    # dataPath = 'G:/EEGDecoding/CFANet2/data/deap/originalData/s01.mat'
    eeg = loadmat(dataPath)
    data = eeg['data'][:,0:32,:] # trial,channel,time
    labels = eeg['labels'][:,0:2]
    # mean = np.expand_dims(np.mean(data,axis=1),1)
    # std = np.expand_dims(np.std(data,axis=1),1)
    # data = (data-np.tile(mean,(1,data.shape[1],1)))/np.tile(std,(1,data.shape[1],1))
    N,C,T = data.shape
    fs = 128
    ##Average reference
    # data = data-np.tile(np.expand_dims(np.mean(data,1),1),(1,C,1))
    # ##baseline correction
    # data = data[:,:,fs*3:fs*63]-np.tile(np.expand_dims(np.mean(data[:,:,fs*2:fs*3],2),2),(1,1,fs*60))
    data = data[:,:,fs*3:fs*63]
    #Epoch the data 
    data = data.transpose(1,2,0) # channel,time,trial
    ws = 2
    step = 1
    x = np.zeros([C,fs*ws,N*(int((60-ws)/step)+1)])
    for n in range(N*(int((60-ws-1)/step)+1)):
        x[:,:,n] = data[:,int((n%((60-ws)/step+1))*step*fs):int((n%((60-ws)/step+1))*step*fs+ws*fs),int(n/((60-ws)/step+1))]
    # x = np.zeros([C,fs*2,N*30])
    # for n in range(N*30):
    #     x[:,:,n] = data[int(n/30),:,(n%30)*2*fs:(n%30+1)*2*fs]
    
    #label transform
    y = np.zeros([N,1],dtype=np.int32)
    for n in range(labels.shape[0]):
        if labels[n,0]<5 and labels[n,1]<5:
            y[n]=0
        elif labels[n,0]<5 and labels[n,1]>=5:
            y[n]=1
        elif labels[n,0]>=5 and labels[n,1]<5:
            y[n]=2
        elif labels[n,0]>=5 and labels[n,1]>=5:
            y[n]=3
    y = np.tile(y,(1,int((60-ws)/step+1))).reshape(-1)  
    
    data = {'x': x, 'y': y, 'c': chans, 's': fs}
    return data

def parseDeapDataset(datasetPath, savePath, 
                       epochWindow = [0,2], chans = list(range(40)), verbos = False):
    '''
    Parse the BCI comp. IV-2a data in a MATLAB formate that will be used in the next analysis
    ...
    ----------
    datasetPath : str
        Path to the DEAP original dataset in mat formate.
    savePath : str
        Path on where to save the epoched eeg data in a mat format.
    epochWindow : list, optional
        time segment to extract in seconds. The default is [0,4].
    ...
    subjects=[]
    for s in range(1,33):
        subjects.append('s'+str(s).zfill(2))
    # subjects=['A01T','A02T','A03T','A04T','A05T','A06T','A07T','A08T','A09T']
    # test_subjects=['A01E','A02E','A03E','A04E','A05E','A06E','A07E','A08E','A09E']
    # subAll = [subjects, test_subjects]
    # subL = ['s', 'se'] # s: session 1, se: session 2 (session evaluation)
    
    print('Load data in mat format: ')
    if not os.path.exists(savePath):
        os.makedirs(savePath)
    print('Processed data be saved in folder : ' + savePath)
    ...
    for subs in range(len(subjects)):
        sub = subjects[subs]
        if not os.path.exists(os.path.join(datasetPath, sub+'.mat')):
            raise ValueError('The DEAP original dataset doesn\'t exist at path: ' + 
                              os.path.join(datasetPath, sub+'.mat') + 
                              ' Please download and copy the extracted dataset at the above path '+
                              ' More details about how to download this data can be found in the Instructions.txt file')
            
        print('Processing subject No.: ' + sub)
        data = parseDeapFile(os.path.join(datasetPath, sub+'.mat'), 
                             chans = chans)
        savemat(os.path.join(savePath, 's'+str(subs+1).zfill(3)+'.mat'), data)
   ...
   # Check if the processed mat data exists:
    if not os.path.exists(os.path.join(dataFolder, rawMatFolder)):
        ...
        if datasetId ==0:
            ...
            parseDeapDataset(os.path.join(dataFolder, oDataFolder), os.path.join(dataFolder, rawMatFolder))
    ...
    if not os.path.exists(os.path.join(dataFolder, multiviewPythonFolder , 'dataLabels.csv')):
        ...
        if datasetId ==0 or datasetId ==1:
            pythonToMultiviewPython( os.path.join(dataFolder, rawPythonFolder), os.path.join(dataFolder, multiviewPythonFolder))
        elif datasetId ==2:
            pythonToMultiviewPython( os.path.join(dataFolder, rawPythonFolder), os.path.join(dataFolder, multiviewPythonFolder),
                                    filterTransform = {'filterBank':{'filtBank':[[4,8],[8,12],[12,16],[16,20],[20,24],[24,28],[28,32],[32,36],[36,40]],'fs':128, 'filtType':'filter'}})
