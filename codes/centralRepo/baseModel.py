...
class baseModel():
    ...
    def train(
        ...
        trainResults = self._trainOE(trainData, valData, lossFn,
                                     ...
                                     centerloss=centerloss,optimzer4center=optimzer4center)
    ...
    def _trainOE(
        ...
        while not doStop:
            ...
            loss.append(self.trainOneEpoch(trainData, lossFn, self.optimizer, sampler = sampler, centerLossEnable=centerLossEnable,
                                     centerloss=centerloss,optimzer4center=optimzer4center,Lambda=0.0005))
    ...
    def trainOneEpoch(self, trainData, lossFn, optimizer, sampler = None,centerLossEnable=False,
        centerloss=None, optimzer4center=None,Lambda=0):
        '''
        Train for one epoch
        ...
        with torch.no_grad():
            ...
            for d in dataLoader:
                ...
                if tsneEnable:
                    ...
                    plt.savefig(filepath + '.jpg', dpi=600)
                    plt.show()
    ...
    def calculateResults(self, yPredicted, yActual, classes = None):
        ...
        f1 = f1_score(yActual, yPredicted, average='macro')
        kappa = cohen_kappa_score(yActual, yPredicted)
        if classes is not None:
            cm = confusion_matrix(yActual, yPredicted, labels= classes)
        else:
            cm = confusion_matrix(yActual, yPredicted)

        return {'acc': acc, 'f1': f1, 'kappa': kappa, 'cm': cm}
