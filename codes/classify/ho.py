...
def ho(datasetId = None, network = None, nGPU = None, subTorun=None, centerLossEnable=False):
    ...
    if config['network'] in ['FBMSNet','FBMSNet_Inception','FBNASNet','FBCNet']:
   ...
   #%% Find all the subjects to run 
    ...
    for iSub, sub in enumerate(subs):
        ...
        # NasVal = copy.deepcopy(trainData)
        # NasTrain = copy.deepcopy(trainData)
        # NasVal.createPartialDataset(list( range( 
        #     math.ceil(len(trainData)*(0.5)) , len(trainData))))       
        # NasTrain.createPartialDataset(list(range(0, math.ceil(len(trainData)*(0.5)))))
        ...
        if modeInFol == 'multiviewPython':
            if datasetId == 0:
                input = torch.randn(1, 9, 22, 1000)
            if datasetId == 1:
                input = torch.randn(1, 9, 20, 1000)
        else:
            if datasetId == 0:
                input = torch.randn(1, 1, 22, 1000)
            if datasetId == 1:
                input = torch.randn(1, 1, 20, 1000)
        ...
        if config['network'] == 'FBNASNet':
            SuperNet = networks.__dict__['SuperNet']
            supernet = SuperNet(**config['modelArguments'])
            start = time.time()
            if datasetId == 1:
                opt_choice = nas_phase(supernet,trainData, valData, classes=2,supernet_path=os.path.join(outPathSub,'supernet.pth'))
            if datasetId == 0:   
                opt_choice = nas_phase(supernet,trainData, valData, classes=4,supernet_path=os.path.join(outPathSub,'supernet.pth'))
            SearchTime_list[str(sub)] = str(time.time()-start)
            dictToCsv(os.path.join(outPathSub,'opt_choice.csv'), opt_choice)
            net = network(**config['modelArguments'],opt_choice = opt_choice)
        ...
        FLOPs_list[str(sub)] = str(Flops / 1000000000)
        params_list[str(sub)] = str(params / 1000000)
        
        model = baseModel(net=net, resultsSavePath=outPathSub, batchSize= config['batchSize'], nGPU = nGPU)
        model.train(trainData, valData, testData, **config['modelTrainArguments'],centerLossEnable=config['centerLossEnable'])
    ...
    for i, res in enumerate(trainAcc):
        ...
        sheet1 = excelAddData(sheet1, [0,0], ['SubId', 'trainAcc', 'valAcc', 'testAcc','testF1','testKappa'])
        ...
        sheet1 = excelAddData(sheet1, [1,4], [[f1] for f1 in testF1[i]], isNpData= True)
        sheet1 = excelAddData(sheet1, [1,5], [[kappa] for kappa in testKappa[i]], isNpData= True)
    ...
    dictToCsv(os.path.join(config['outPath'],'FLOPs.csv'), FLOPs_list)
    dictToCsv(os.path.join(config['outPath'],'parameters.csv'), params_list)
    dictToCsv(os.path.join(config['outPath'],'SearchTime.csv'), SearchTime_list)
...
if __name__ == '__main__':
    ...
    if count >3:
        ...
        subTorun = None
    ho(datasetId=0, network='FBNASNet', nGPU='0', subTorun=subTorun, centerLossEnable=False)
